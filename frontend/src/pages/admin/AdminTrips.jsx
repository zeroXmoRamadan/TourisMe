...
const AdminTrips = () => {
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            const tripsData = await tripPlannerService.getAllTrips();
            setTrips(tripsData);
            const statsData = await tripPlannerService.getAdminStats();
            setStats(statsData);
        } catch (error) {
                ...
                <div ref={contentRef}>
                    <div className="flex flex-col md:flex-row gap-4 mb-6">
                        ...
                        <div className="flex gap-2 flex-wrap">
                            {['all', 'planning', 'confirmed', 'completed'].map(f => (
                                        ...
                                        return (
                                            <tr key={trip.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                                                ...
                                                <td className="py-4 px-4 text-right">
                                                    <button onClick={() => setViewTrip(trip)} className="p-2 hover:bg-primary-500/10 rounded-lg transition-colors" title="View Details"><Eye className="w-4 h-4 text-primary-400" /></button>
                                                </td>
                                            </tr>
                                        );
                                    })}
                        ...
                        <Card className="relative z-10 w-full max-w-2xl">
                            ...
                            <div className="space-y-2">
                                {(viewTrip.items || []).map(item => {
                                    ...
                                    const CatIcon = detailIcons[item.type] || Package;
                                    return (
